...
inline sim::AgentSampler<sim::World> robots() {
  ...
  group.behavior.safety_margin = make_const(static_cast<ng_float_t>(0.5));
  group.behavior.properties["tau"] =
      make_const_property(static_cast<ng_float_t>(0.25));
  ...
  group.position = std::make_unique<sim::RegularSampler<Vector2>>(
      sim::RegularSampler<Vector2>::make_with_interval({0, 0}, {10, 0},
                                                       number));
...
inline sim::AgentSampler<sim::World> group(const std::string &behavior,
                                           int number) {
  sim::AgentSampler<sim::World> group;
  group.id = make_const(1);
  group.number = make_const<unsigned>(number);
