...
class Node(rclpy.node.Node):  # type: ignore
    ...
    def __init__(self) -> None:
        ...
        coords: List[float] = self.declare_parameter("path", [0.0, 0.0]).value
        if len(coords) % 2 != 0:
            self.get_logger().warning("Number of coordinates {coords} should be even!")
            coords.append(0.0)
        self.path: List[Tuple[float, float]] = list(zip(coords[::2], coords[1::2]))

        self.client = rclpy.action.ActionClient(
            self, navground_msgs.action.GoToTarget, 'go_to_target')
